...
import MuscleBodyDiagram from '@/components/ui/MuscleBodyDiagram'
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      <div className="forge-stats-grid">
        {[
          { label: 'Calories today', value: `${Math.round(totals.calories)}`, sub: `of ${calTarget} kcal target`, color: calPct > 1 ? 'var(--danger)' : calPct > 0.85 ? 'var(--warning)' : 'var(--success)', href: '/meals' },
          { label: 'Protein', value: `${Math.round(totals.protein_g)}g`, sub: `of ${profile?.protein_target_g ?? 180}g target`, color: 'var(--protein-color)', href: '/meals' },
          { label: "Today's workout", value: todayWorkout ? '✅ Logged' : '—', sub: todayWorkout?.name ?? 'No session logged yet', color: undefined, href: '/workouts' },
          { label: 'Current weight', value: weightLogs[0] ? `${weightLogs[0].weight_kg}kg` : '—', sub: weightLogs[0] ? weightLogs[0].date : 'Not logged', color: undefined, href: '/progress' },
        ].map(s => (
          <div key={s.label} onClick={() => router.push(s.href)} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '12px', padding: '16px', cursor: 'pointer' }}>
      ...
      <div className="forge-two-col">
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '12px', padding: '20px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
            ...
            <button onClick={() => router.push('/meals')} style={{ fontSize: '12px', color: 'var(--accent)', background: 'none', border: 'none', cursor: 'pointer' }}>+ Log meal</button>
          </div>
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '12px', padding: '20px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
            ...
            <button onClick={() => router.push('/progress')} style={{ fontSize: '12px', color: 'var(--accent)', background: 'none', border: 'none', cursor: 'pointer' }}>+ Log weight</button>
          </div>
          {weightData.length > 1 ? (
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '12px', padding: '20px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
          <div>
            <h2 style={{ fontSize: '15px', fontWeight: '600' }}>Muscle activity — last 7 days</h2>
            <p style={{ fontSize: '12px', color: 'var(--text-secondary)', marginTop: '2px' }}>Colour intensity shows training frequency</p>
          </div>
          <button onClick={() => router.push('/workouts')} style={{ fontSize: '12px', color: 'var(--accent)', background: 'none', border: 'none', cursor: 'pointer' }}>Log workout</button>
        </div>
        <MuscleBodyDiagram muscleHits={muscleHits} sex={profile?.sex ?? 'male'} />
      </div>
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '12px', padding: '20px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '12px', marginBottom: '16px', flexWrap: 'wrap' }}>
          ...
          <button onClick={getAiWorkout} disabled={aiLoading} style={{
        ...
        {aiWorkout && (
          <div style={{ borderTop: '1px solid var(--border)', paddingTop: '16px' }}>
            ...
            <button onClick={() => router.push('/workouts?from=ai')} style={{ marginTop: '16px', padding: '10px 20px', background: 'var(--success-subtle)', border: '1px solid var(--success)', color: 'var(--success)', borderRadius: '8px', fontSize: '13px', fontWeight: '500', cursor: 'pointer' }}>
              Start this workout →
            </button>
          </div>
        )}
      ...
      <style>{`
        .forge-stats-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; }
        .forge-two-col { display: flex; flex-direction: column; gap: 16px; }
        @media (min-width: 768px) {
          .forge-stats-grid { grid-template-columns: repeat(4, 1fr); }
          .forge-two-col { flex-direction: row; }
          .forge-two-col > * { flex: 1; }
        }
      `}</style>
    </div>
  )
